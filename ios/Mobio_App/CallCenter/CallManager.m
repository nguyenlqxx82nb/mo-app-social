...
//  [self checkTokenAvailable:@"" completionHandler:^(BOOL status) {
//    NSLog(@"handleIncomingPushEvent checkTokenAvailable status= %@", status ? @"yes" : @"no" );
//    if (!status) {
//      return;
//    }
//
...
//  [self checkTokenAvailable:@"" completionHandler:^(BOOL status) {
//    NSLog(@"checkTokenAvailable status= %@", status ? @"yes" : @"no" );
//    if (!status) {
//      return;
//    }
//
...
- (void)checkTokenAvailable:(NSString*)demo completionHandler:(void(^)(BOOL error))completionHandler {
  NSUserDefaults* prefs = [NSUserDefaults standardUserDefaults];
  NSString* host = [prefs valueForKey:@"HOST_ADMIN"];
  NSString* token = [prefs valueForKey:@"TOKEN"];
  NSString* urlString = [NSString stringWithFormat:@"%@jwt/action/status?jwt=Bearer%@%@", host, @"%20",token];
  NSURL *url = [NSURL URLWithString:urlString];
  NSMutableURLRequest *request = [[NSMutableURLRequest alloc] initWithURL:url];
  [request setHTTPMethod:@"GET"];
  
  [request setValue:@"application/json" forHTTPHeaderField:@"Content-Type"];
  NSLog(@"checkTokenAvailable urlString= %@ host= %@", urlString , host);
  ...
  [[[NSURLSession sharedSession] dataTaskWithRequest:request completionHandler:
    ...
    if (error) {
      NSLog(@"checkTokenAvailable ERROR=%@",error);
      return;
    }
    ...
    long status = [dictionary[@"status"] integerValue];
    NSLog(@"checkTokenAvailable status=%ld, cod=%ld", status, code);
    if (code != 200) {
